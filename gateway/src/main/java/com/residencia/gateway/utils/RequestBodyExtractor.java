...
public class RequestBodyExtractor {
    ...
    public GatewayRequest getRequest(ServerWebExchange exchange, DataBuffer buffer) {
        ...
        // Set headers -- Needed:
        // https://github.com/spring-cloud/spring-cloud-gateway/issues/894
        /*
        ...
        processedHeaders.putAll(exchange.getRequest().getHeaders());

        // Agregar headers personalizados si existen
        if (request.getHeaders() != null) {
            request.getHeaders().forEach(processedHeaders::set);
        }

        processedHeaders.remove(HttpHeaders.CONTENT_LENGTH);
        processedHeaders.set(HttpHeaders.TRANSFER_ENCODING, "chunked");
        request.setProcessedHeaders(processedHeaders);
        return request;
    }
